...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            self.trial_no += 1
            self.epsilon = math.cos(self.alpha*self.trial_no)
    ...
    def createQ(self, state):
        ...
        if self.learning:
            if state not in self.Q.keys():
                new_states = {}
                new_states = new_states.fromkeys(self.valid_actions, 0.0)
                self.Q[state] = new_states
        return
	...
	if not self.learning:
            ...
            eps_val  = random.random() < 1 - self.epsilon
            if not eps_val:
                valid_actions = filter(lambda x: x != max(action_possible.items(), key=lambda x: x[1])[0], self.valid_actions)
                action = random.choice(valid_actions)
            else:
                action = max(action_possible.items(), key=lambda x: x[1])[0]
